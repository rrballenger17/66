...
public class InsertStatement extends SQLStatement {
    ...
    public void execute() throws DatabaseException, DeadlockException {
     ...
     // row.getData()
            ...
            DatabaseEntry keyEntry = row.getKey();
            
            if(keyEntry != null){
              
              if(db.putNoOverwrite(null, keyEntry, row.getData()) == OperationStatus.KEYEXIST)
                throw new DatabaseException("Primary key already exists");
              
            }else{
            
              db.append(null, new DatabaseEntry(), row.getData());
            }
     //if(true) throw new Exception("added right?");

     //System.out.println("/n added to database /n");
